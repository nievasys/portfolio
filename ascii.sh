...
video_to_terminal() {
    ...
    echo "Publishing frames to public/frames/..."
    local public_frames_dir="public/frames"
    mkdir -p "$public_frames_dir"
    rm -f "$public_frames_dir"/frame_*.txt "$public_frames_dir"/frames.json
    cp "$working_dir"/frame_images/frame_*.txt "$public_frames_dir/"

    local frame_count
    frame_count=$(ls "$public_frames_dir"/frame_*.txt | wc -l)
    printf '{"count": %d}\n' "$frame_count" > "$public_frames_dir/frames.json"
    echo "Published $frame_count frames and manifest ($public_frames_dir/frames.json)."
